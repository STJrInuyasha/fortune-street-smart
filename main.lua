	...
	function drawSquare(square)
		...
		if (SquareTypes[square.type].districts) and DistrictColors[square.district] then
	...
	function drawSquareMovements(square)
		...
		-- Destination square for warps, stairs, etc
		-- Technically a "movement" I guess
		if SquareTypes[square.type].destinations then
	...
	function drawSquareDetails(square)
		...
		local squareType = SquareTypes[square.type] and SquareTypes[square.type].name or "UNKNOWN"
		love.graphics.print(string.format("%s\n%s\nPos: [%d, %d, %d]",
			square.name, squareType, square.xPos, square.yPos, square.zPos),
			15, dHeight + 5)

		if square.type == 1 or (square.value ~= 0 or square.price ~= 0) then
			love.graphics.print(string.format("Value: %d\nPrices: %d", square.value, square.price), 15, dHeight + 55)
		end

		-- Display district; the square may be in another district too,
		-- but this is the district (color) that gets displayed
		if SquareTypes[square.type].districts then
			love.graphics.print(string.format("District %d (%s)", square.district, districts[square.district].name), 15, dHeight + 95)
		end

		love.graphics.print(square["moveMaskAll"]:gsub("(%d+ %d+) ", "%1\n"):gsub("0", "- "), 15, dHeight + 125)

		love.graphics.print(square["extra"]:hexDump(), 15, dHeight + 400)
